...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): https://sgc.ai/docs/config#precedence-order
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://sgc.ai/config.json"
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://sgc.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://sgc.ai/docs/agents"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://sgc.ai/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://sgc.ai/config.json"
        const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://sgc.ai/config.json",')
